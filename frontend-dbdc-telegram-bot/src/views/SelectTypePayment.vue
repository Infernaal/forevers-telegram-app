<template>
  <div class="w-full min-h-screen bg-white font-montserrat flex flex-col">
    ...
    <div class="fixed left-1/2 transform -translate-x-1/2 z-[9999]" :style="{ bottom: `${bottomOffset + 120}px` }">
      <TermsCheckbox
...
// TON Connect state
const { isConnected: tonConnected, accountAddress, connect: tonConnect, sendTransaction: tonSendTx, init: tonInit } = useTonConnect()
...
const actionButtonLabel = computed(() => {
  if (selectedPayment.value === 'crypto') {
    return tonConnected.value ? 'Buy Forevers' : 'Connect Wallet'
  }
  return 'Buy Forevers'
})
...
const handlePurchase = async () => {
  ...
  if (selectedPayment.value === 'crypto') {
    try {
      ...
      // Wait up to 5s for connection state
      const deadline = Date.now() + 5000
      while (!tonConnected.value && Date.now() < deadline) {
      ...
      showApiError('crypto_init', { status: 400, message: e?.message || 'Failed to start crypto payment' })
    }
    return
  }
...
const startTonPurchase = async () => {
  if (!tonConnected.value || !accountAddress.value) {
  ...
  try {
    ...
    if (!tx || !tx.messages || !tx.validUntil) {
      throw new Error('Invalid transaction payload from server')
    }
    await tonSendTx(tx)
    if (verifyId) {
