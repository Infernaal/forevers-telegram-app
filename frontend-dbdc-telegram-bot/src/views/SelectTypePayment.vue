  ...
  <div class="w-full min-h-screen bg-white font-montserrat flex flex-col">
    ...
    <div class="flex-1 bg-gray-100 rounded-t-2xl p-4 mt-20">
      ...
      <div class="grid grid-cols-2 gap-3 mb-8">
        <!-- Loyalty Program -->
        <div 
          class="bg-white border border-gray-200 rounded-xl p-4 flex flex-col items-center gap-2 cursor-pointer transition-all"
          :class="selectedPayment === 'loyalty' ? 'border-dbd-primary bg-blue-50' : ''"
        ...
        <!-- USDT Crypto Wallet -->
        <div 
          class="bg-white border border-gray-200 rounded-xl p-4 flex flex-col items-center gap-2 cursor-pointer transition-all col-span-1"
          :class="selectedPayment === 'usdt' ? 'border-dbd-primary bg-blue-50' : ''"
      ...
      <!-- Total to Pay -->
      <div class="flex items-center justify-center gap-1 mb-6 mt-auto">
        <span class="text-xl font-semibold text-dbd-dark">Total to pay:</span>
        <span class="text-xl font-semibold text-dbd-primary">${{ totalAmount }}</span>
      </div>
      ...
      <button
        ...
        class="w-full bg-gradient-to-r from-dbd-primary to-blue-600 text-white font-bold text-xl py-4 rounded-full transition-all"
        :class="(!selectedPayment || !termsAccepted) ? 'opacity-50 cursor-not-allowed' : 'hover:opacity-90'"
      >
        Buy Forevers
      </button>
    </div>
