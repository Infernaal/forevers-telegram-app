    ...
    <div class="w-full min-h-screen bg-gray-100 relative px-4 pt-5 pb-32 sm:pb-36 md:pb-40 lg:pb-44">
      ...
      <div class="relative overflow-visible w-full px-6 h-96 mb-5">
        ...
        <div class="absolute inset-0 rounded-3xl bg-gradient-to-r from-blue-900 via-indigo-900 to-blue-800 backdrop-blur-lg overflow-hidden">
          ...
          <!-- F_Icon 1 - Top left -->
          <svg class="absolute w-16 h-16 opacity-10 transform rotate-[46.8deg] -left-2 top-4" viewBox="0 0 120 131" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g opacity="1">
              <g clip-path="url(#clip0_f_icon_1)">
          ...
          <svg class="absolute w-20 h-24 opacity-10 transform rotate-[-31.597deg] -right-5 bottom-44" viewBox="0 0 155 177" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <defs>
              <clipPath id="clip0_f_icon_2">
                <rect x="-21.6904" y="67.6526" width="128" height="128" rx="6.4" transform="rotate(-31.5967 -21.6904 67.6526)" fill="white"/>
              </clipPath>
            </defs>
          </svg>

          <svg class="absolute -left-7 top-44 w-36 h-44 opacity-10 transform -rotate-12" viewBox="0 0 155 177" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g opacity="1">
        ...
        <div class="absolute left-1/2 bottom-16 transform -translate-x-1/2 z-20">
          <TermsCheckbox
        ...
        <div class="absolute left-0 right-0 bottom-2 px-4 z-20">
          <button
            ...
            :class="{
              '!bg-gray-400 !bg-none !text-white !cursor-not-allowed': !termsAccepted,
      ...
      <div class="relative w-full mt-12">
        ...
        <div class="flex flex-col items-start gap-3 mb-8">
          <h2 class="text-dbd-dark text-xl font-bold leading-6 m-0">How to use the Link</h2>
          <p class="text-dbd-gray text-base font-medium leading-6 m-0">Follow these 3 simple steps</p>
        </div>
      ...
      <div class="w-full max-w-2xl mx-auto px-4 mb-12">
        ...
        <div class="grid grid-cols-5 gap-0 items-center text-center mb-4">
          ...
          <div class="flex justify-center">
            <div class="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-dbd-primary rounded-r-full flex items-center justify-center">
            ...
            <div class="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-dbd-primary rounded-r-full flex items-center justify-center">
              <div class="flex items-center gap-1">
                <svg width="10" height="10" viewBox="0 0 24 24" fill="none">
                  <path d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM17 13H11V7H13V11H17V13Z" fill="white"/>
                </svg>
          ...
          <div class="flex justify-center">
            <div class="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-dbd-primary rounded-r-full flex items-center justify-center">
              <div class="flex items-center gap-1">
                <svg width="10" height="10" viewBox="0 0 24 24" fill="none">
                  <path d="M12 2L15.09 8.26L22 9L17 13.74L18.18 20.74L12 17.77L5.82 20.74L7 13.74L2 9L8.91 8.26L12 2Z" fill="white"/>
                </svg>
        ...
        <div class="relative w-full h-52">
          ...
          <div class="relative z-10 p-5">
            ...
            <div class="relative w-full max-w-sm h-13 sm:h-14 md:h-16 rounded-full border border-blue-600 border-opacity-60 bg-white bg-opacity-10 flex items-center transition-all duration-400 ease-out overflow-hidden"
              ...
              <!-- Copied State -->
              <div v-if="linkCopied" class="link-copied-content">
                <span class="link-copied-text">Copied</span>
...
const copyWebLink = async () => {
  ...
  try {
    ...
    console.warn('Using cached link:', error.message)
  }

  // Unified copy function with fallback chain
  const performCopy = async (text) => {
    // 1. Telegram WebApp API (most reliable in Telegram)
    if (window.Telegram?.WebApp?.writeTextToClipboard) {
      try {
        await window.Telegram.WebApp.writeTextToClipboard(text)
        return true
      } catch (e) {
        console.log('Telegram clipboard failed:', e.message)
      }
    }

    // 2. Modern Clipboard API (HTTPS required)
    if (navigator.clipboard?.writeText && window.isSecureContext) {
      try {
        await navigator.clipboard.writeText(text)
        return true
      } catch (e) {
        console.log('Modern clipboard failed:', e.message)
      }
    }

    // 3. Legacy execCommand method (universal fallback)
    try {
      const textarea = document.createElement('textarea')
      textarea.value = text
      ...
      console.error('Legacy copy failed:', e.message)
      return false
    }
  }

  // Execute copy operation
  const copySuccess = await performCopy(linkToCopy)
  ...
  // User feedback
  showSuccessMessage(copySuccess ? 'Link copied to clipboard' : 'Copy failed, please try again')
...
/* Copied State Styling - matching ProfileOverlay */
.link-copied-state {
...
/* Animations from ProfileOverlay */
