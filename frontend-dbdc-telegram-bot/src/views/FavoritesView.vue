...
const calculateCardOpacities = () => {
  if (!scrollContainer.value || !cardRefs.value.length) return

  const containerRect = scrollContainer.value.getBoundingClientRect()

  // Define fade zones
  const foreversBalanceHeight = 140 // Approximate height of ForeversBalance block
  const bottomNavHeight = 100 // Approximate height of BottomNavigation
  const fadeDistance = 50 // Distance over which to fade

  // Calculate top and bottom fade zones
  const topFadeStart = containerRect.top + foreversBalanceHeight - fadeDistance
  const topFadeEnd = containerRect.top + foreversBalanceHeight + fadeDistance

  const bottomFadeStart = containerRect.bottom - bottomNavHeight - fadeDistance
  const bottomFadeEnd = containerRect.bottom - bottomNavHeight + fadeDistance
  ...
  cardRefs.value.forEach((card, index) => {
    ...
    // Check if card is in top fade zone (behind ForeversBalance)
    if (cardCenter >= topFadeStart && cardCenter <= topFadeEnd) {
      const progress = (cardCenter - topFadeStart) / (topFadeEnd - topFadeStart)
      opacity = Math.max(0, Math.min(1, progress))
    } else if (cardCenter < topFadeStart) {
      opacity = 0
    }

    // Check if card is in bottom fade zone (behind BottomNavigation)
    if (cardCenter >= bottomFadeStart && cardCenter <= bottomFadeEnd) {
      const progress = (bottomFadeEnd - cardCenter) / (bottomFadeEnd - bottomFadeStart)
      opacity = Math.min(opacity, Math.max(0, Math.min(1, progress)))
...
const getCardStyle = (index) => {
  const opacity = cardOpacities.value[index] ?? 1
  return {
    opacity,
...
const initializeCardOpacities = () => {
  cardOpacities.value = new Array(balances.value.length).fill(1)
  nextTick(() => {
    calculateCardOpacities()
  })
}
