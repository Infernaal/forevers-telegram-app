<template>
  <div class="cart-view w-full h-full bg-gray-100 min-h-screen flex flex-col font-montserrat" style="background: #f3f4f6 !important;">
    ...
    <div class="content-container bg-gray-100 flex-1 flex flex-col relative">
      ...
      <div v-else class="flex-1 flex items-center justify-center xs:px-3 sm:px-3.5 md:px-5 lg:px-6 xs:pt-3 sm:pt-4 md:pt-5">
        <div class="text-center">
          <div class="xs:w-12 xs:h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 bg-gray-200 rounded-lg flex items-center justify-center xs:mb-3 sm:mb-4 md:mb-5 lg:mb-6 mx-auto">
            <svg width="20" height="22" viewBox="0 0 18 20" class="text-gray-400 xs:w-4 xs:h-5 sm:w-5 sm:h-6 md:w-6 md:h-7 lg:w-7 lg:h-8">
          ...
          <h2 class="xs:text-lg sm:text-xl md:text-2xl lg:text-3xl font-semibold text-gray-900 xs:mb-1 sm:mb-2 md:mb-3">Your cart is empty</h2>
          <p class="text-gray-500 xs:text-sm sm:text-base md:text-lg xs:mb-4 sm:mb-6 md:mb-8">Add some Forevers to get started!</p>
          <button
            @click="$router.push('/favorites')"
            class="bg-dbd-primary text-white font-semibold xs:py-2 xs:px-4 sm:py-3 sm:px-6 md:py-4 md:px-8 xs:text-sm sm:text-base md:text-lg rounded-full hover:bg-blue-700 transition-colors"
          >
            Browse Forevers
          </button>
        </div>
