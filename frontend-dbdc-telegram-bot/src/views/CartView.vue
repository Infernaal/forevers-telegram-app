    ...
    <div class="px-4 pt-4 pb-52 sm:pb-56 md:pb-80 lg:pb-72 xl:pb-80 min-h-[calc(100vh-100px)] flex-1 flex flex-col">
      ...
      <div v-else class="fixed inset-0 top-4 bottom-[84px] sm:bottom-[92px] flex items-center justify-center px-4">
        <div class="text-center w-full max-w-sm mx-auto">
          ...
          <h2 class="text-xl sm:text-2xl font-semibold text-gray-900 mb-3">Your cart is empty</h2>
          <p class="text-gray-500 mb-8 text-base sm:text-lg">Add some Forevers to get started!</p>
          <button
            @click="$router.push('/favorites')"
            class="bg-dbd-primary text-white font-semibold py-3 px-8 rounded-full hover:bg-blue-700 transition-colors text-base sm:text-lg w-full sm:w-auto"
          >
            Browse Forevers
          </button>
        </div>
...
@media (min-width: 640px) {
  .cart-bottom-container {
    bottom: 110px; /* tablet - уменьшили с 100px */
  }
}
...
@media (min-width: 1024px) {
  .cart-bottom-container {
    bottom: 120px; /* desktop - уменьшили с 120px */
  }
}

@media (min-width: 1280px) {
  .cart-bottom-container {
    bottom: 130px; /* desktop large - уменьшили с 130px */
  }
}
