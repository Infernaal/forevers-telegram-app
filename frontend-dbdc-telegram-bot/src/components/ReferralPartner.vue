<template>
  <div class="w-full h-12 rounded-lg bg-white bg-opacity-50 flex items-center px-3 gap-2">
    ...
    <div class="flex-1 flex items-center">
      <span class="text-white text-sm font-medium">Your partner</span>
    </div>

    <!-- Partner Name and Link -->
    <div class="flex flex-col items-end">
      <span class="text-white text-base font-medium leading-5">
        {{ partnerName }}
      </span>
      <a 
        :href="partnerLink" 
        class="text-dbd-primary text-sm font-semibold leading-4 underline"
        target="_blank"
...
const partnerName = computed(() => {
  if (props.partnerFirstName && props.partnerLastName) {
    return `${props.partnerFirstName} ${props.partnerLastName}`
  }
  return 'Unknown Partner'
})

const partnerDisplayId = computed(() => {
  if (props.partnerUserId) {
    return `vm.dubadu/${props.partnerUserId}`
  }
  return 'vm.dubadu/unknown'
})

const partnerLink = computed(() => {
  if (props.partnerUserId) {
    return `https://vm.dubadu/${props.partnerUserId}`
  }
  return '#'
})
</script>
